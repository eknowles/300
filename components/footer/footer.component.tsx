...
const data = [
  {
    title: 'Featured Communities', links: [
  ...
  {
    title: 'Supported Games', links: [
      { label: 'Hell Let Loose', href: '/games/hell-let-loose', disabled: false },
  ...
  },
  {
    title: 'Developers', links: [
      ...
      { label: 'Integrations', href: '/developers/integrations', disabled: true },
    ]
  },
];
        ...
        <List
          ...
          renderItem={item => (
            <List.Item>
              ...
              {item.links.map((i) => (
                <div key={i.href}>
                  {i.disabled ? <Text disabled>{i.label}</Text> : <Link href={i.href}><a>{i.label}</a></Link>}
                </div>
              ))}
            </List.Item>
          )}
